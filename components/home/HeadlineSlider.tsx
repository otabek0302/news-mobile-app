...
const HeadlineSlider: React.FC<HeadlineSliderProps> = ({ newsList }) => {
  ...
  return (
    <View style={styles.headlineSliderContainer}>
      <FlatList data={newsList} keyExtractor={(item, index) => index.toString()} style={styles.headlineSliderFlatList} renderItem={({ item }) => (
          <View>
            <TouchableOpacity style={styles.headlineSliderImageBox} onPress={() => handleNewsPress(item)}>
              <Image source={{ uri: item.urlToImage }} style={styles.headlineSliderImage} />
